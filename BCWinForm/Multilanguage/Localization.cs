...
namespace com.mirle.ibg3k0.bc.winform
{
    public static class Localization
    {
        ...
        public static LanguageConfigHandler LCH = null;
        ...
        public static bool buildResource = false;
        public static bool changeFont = false;
        public static bool genTranslateKeys = false;
                        ...
                        if (element.GetAttribute("name").EndsWith(".Text") || element.GetAttribute("name").EndsWith(".HeaderText"))//
                        {
                            foreach (XmlNode childNode in element.ChildNodes)
                            {
                                if (childNode.Name == "value")
                                {
                                    ...
                                    //if (!keyList.Exists(x => x.ToLower().Replace(" ", "") == text.ToLower().Replace(" ", ""))) //修正Key因為結尾有\r\n而重複問題
                                    if (!keyList.Exists(x => x.ToLower().Trim().Replace(" ", "") == text.ToLower().Trim().Replace(" ", "")))
                                    {
        ...
        public static void BuildMultiLanguageResources(string bc_id)
        {

            LCH = new LanguageConfigHandler(bc_id);
            buildResource = LCH.getBuildFlag();
            changeFont = LCH.getFontFlag();
            genTranslateKeys = LCH.getGenKeysFlag();
            if (genTranslateKeys)
            {
            ...
            if (buildResource)//根據config buildResource決定是否建立資源檔
            {
                ...
                foreach (Type t in types)                    //找尋組件內所有類別型態
                {
                    //自動取得所有Form來建立資源檔
                ...
                foreach (string filePath in System.IO.Directory.GetFiles(System.IO.Directory.GetCurrentDirectory() + @"\Language"))//對每個在Language資料夾裡的語言檔案件進行處理
                {
                    ...
                    foreach (FileInfo resxFile in resxFileArray)
                    {
                        ...
                        if (containerName != null)
                        {
                            ...
                            alResources += (@" /embed:" + "\"" + resourcefileName + "\"");//檔案路聽加上雙引號，避免路徑出現空白造成斷句問題
                        }
        ...
        public static void generateResourceDll()
        {
            ...
            File.WriteAllText(dir + @"\" + "al.txt", @"/t:lib" + alResources + @" /culture:" + Lang + @" /out:" + "\"" + dir + @"\" + Lang + @"\" + processName + ".resources.dll" + "\"");//檔案路聽加上雙引號，避免路徑出現空白造成斷句問題
        ...
        public static void ResGen(string sourceFilename)
        {
            ...
            resGen.StartInfo.Arguments = "\"" + sourceFilename + "\"" + " " + "\"" + resourcefileName + "\"";//檔案路聽加上雙引號，避免路徑出現空白造成斷句問題
            resGen.Start();
            resGen.WaitForExit();
